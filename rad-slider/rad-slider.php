...
//HTML output. call rad_slider() in your theme to display the slideshow

function rad_slider( $number ){
	
	//get up to 5 slides 
	$slide_query = new WP_Query( array(
		'post_type' 		=> 'slide', //a registered post type (see above)
	...
	//clean up the $post so it doesn't clash with other loops
	wp_reset_postdata();
}

//Attach JS and CSS
function rad_slider_enqueue_stuff(){
	//attach jquery (built-into WP)
	wp_enqueue_script('jquery');
	
	//attach responsiveslides
	$src = plugins_url( 'js/responsiveslides.js', __FILE__ );
							//handle 	  //file  //dependencies
	wp_enqueue_script( 'responsiveslides', $src, 'jquery' );
	
	//attach our custom js
	$src = plugins_url( 'js/custom.js', __FILE__ );
	wp_enqueue_script( 'slides-custom', $src, 'responsiveslides' );

	//attach stylesheet
	$src = plugins_url( 'css/style.css', __FILE__ );
	wp_enqueue_style( 'slides-style', $src );
}

add_action( 'wp_enqueue_scripts', 'rad_slider_enqueue_stuff' );
